...
// const BLOCKS: { id: string; label: string; type: BlockType }[] = [
...
//   block,
// }: {
//   block: { id: string; label: string; type: BlockType };
// }) => {
...
// const CanvasBlock = ({ block, moveBlock, registerRef }: { block: Block; moveBlock: (id: string, x: number, y: number) => void; registerRef: (id: string, ref: HTMLDivElement | null) => void; }) => {
...
//       if (!delta) return;
//       moveBlock(item.id, block.x + delta.x, block.y + delta.y);
//     }
//   });
...
//         drag(drop(node));
//       }}
//       className={`absolute p-2 bg-green-300 rounded cursor-move ${isDragging ? "opacity-50" : ""}`}
...
//     <div
//       ref={ref}
//       className="flex-1 min-h-[500px] bg-white border relative overflow-hidden rounded"
//     >
//       {blocks.map((block) => (
//         <CanvasBlock
//           key={block.id}
...
//           <Canvas blocks={blocks} moveBlock={moveBlock} onDropBlock={handleDropBlock} registerRef={registerRef} />
